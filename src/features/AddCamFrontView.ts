 ...
 *
 */
class AddCamFrontView extends StepInterface {
  envAlias: string = 'Front_Camera';
  camType: string = 'F_CAM';
  ...
  streamFrontInterval: ReturnType<typeof setInterval> | null = null;
  ntwDisconnectFront: boolean = false;
   ...
   */
  start(): void {
    cameraSetup.setPageTitle(ui.translations.cameraSetup.front_camera);
    let self = this;
    this.subscribe(SDK_EVENT.NETWORK_DISCONNECT, function (){
      self.ntwDisconnectFront = true;
    ...
    if (LiveStreamManager.PRIMARY_CAMERA_NAME === 'P_CAM') {
      ...
      liveStreamManager.getCameraStream(LiveStreamManager.CAMERA.FRONT, 'environment')
        .then((stream) => {
          ...
          peer.connect(socketUserName, LiveStreamManager.CAMERA.FRONT);

          // utility.wait(2000).then(() => {
          //   peer.streamAdd(socketUserName, LiveStreamManager.CAMERA.FRONT);
          // });
        })
        .catch((error) => {
          ui.alertDialog(
            ...
            (dialog: HTMLElement) => {
              ui.remove(dialog);
              if (LiveStreamManager.PRIMARY_CAMERA_NAME !== 'P_CAM') {
                const message = { mode: 'front_camera_restart', text: 'front camera restart' };
    ...
    if (button) {
      ui.click(button, async () => {
        ...
        this.subscribe(SDK_EVENT.SECOND_STREAM, function (user_name: string, stream: MediaStream) {
          ...
          cameraSetup.hideCameraSelectPage();
          clearInterval(self.streamFrontInterval!);
        });
        ...
        this.subscribe(SDK_EVENT.SECOND_STREAM_DISCONNET, function (user_name: string) {
          ...
          if(cameraName == 'F_CAM'){
            ...
            if (self.cameraRevokePopup == null) {
              if (LiveStreamManager.CAMERA.FRONT.external == true) {
  ...
  async setRoomStream(stream: MediaStream, aiStart = 1) {
    ...
    cameraSetup.setAttemptData(configrationManager.roomAttemptNo, 3);
    if (stream) {
      ...
      if (aiStart == 1) {
        ...
        utility.log('Starting AI for Front camera');
  ...
  waitForPosition(video: HTMLVideoElement): Promise<any> {
    return new Promise((resolve) => {
      ai.secondaryCameraPosition(video, this.camType, (message: any) => {
        ...
        if(this.ntwDisconnectFront == true){
          return;
        }
        ...
        if (message.status_code === 155 || this.flag) {
          ...
          if (this.validPositionCount > 2 || this.flag) {
            ...
            utility.log(msg, 'stop_Front_camera_position_ai')
            );
  ...
  waitForValidation(video: HTMLVideoElement): Promise<any> {
    return new Promise((resolve) => {
      ai.secondaryCameraPositionValidate(video, (message: any) => {
        ...
        if (message.status_code === 170 || this.flag) {
  ...
  modeSelector(mode: string, message: Record<string, any>, user_name: string = '') {
    switch (mode) {
      ...
      case 'cam_setup_audioText':
        ...
        break;
      case 'sending_stream':
        this.streamFrontInterval = setInterval(() => { 
      ...
      case 'getting_stream':
        ...
        peer.connect(user_name, LiveStreamManager.CAMERA.FRONT);
        break;
      default:
  ...
  completeCameraSetup(log: boolean = true) {
    ...
    stepUIManager.insertText(
      ...
      ui.translations.status.front_cameraSetupFinish
    );
    ui.hide(ui.id('thinkX_cameraSetup_proceeding'));
    const camSetUp = ui.id('thinkX_frontCameraSetup_Start');
    if (camSetUp) {
  ...
  closeTrigger() {
    ...
    AddCamFrontView.stepManager.closeApplication();
    utility.log('close');
  }
  cameraRevoke(): void {
    let self = this;
    ai.stopSecondaryCameraPosition((msg: any) => {});
    ai.stopSecondaryCameraPositionValidate((msg: any) => {});
    utility.log('front camera revoke alert show');
...
export const addCamFrontView = new AddCamFrontView();
