 ...
 *
 */
class AddCamBackView extends StepInterface {
  envAlias: string = 'Back_Camera';
  camType: string = 'B_CAM';
  ...
  streamBackInterval: ReturnType<typeof setInterval> | null = null;
  ntwDisconnectBack: boolean = false;
   ...
   */
  start(): void {
    cameraSetup.setPageTitle(ui.translations.cameraSetup.back_camera);
    let self = this;
    this.subscribe(SDK_EVENT.NETWORK_DISCONNECT, function (){
      self.ntwDisconnectBack = true;
    ...
    if (LiveStreamManager.PRIMARY_CAMERA_NAME === 'P_CAM') {
      ...
      if (configrationManager.CameraSetupInstruction) {
        ...
        cameraSetup.hideCameraStreamPage();
        ui.triggerEventById('thinkX_CameraSetup_ProceedNow', 'click');
      }
    } else {
      ui.hide(ui.id('thinkX_cameraSetup_Instruction'));
      cameraSetup.hideCountLabelInMobile();
      liveStreamManager.getCameraStream(LiveStreamManager.CAMERA.BACK, 'environment')
        .then((stream) => { 
          if (stream) {
          ...
          peer.connect(socketUserName,LiveStreamManager.CAMERA.BACK);

          // utility.wait(2000).then(() => {
          //   peer.streamAdd(socketUserName, LiveStreamManager.CAMERA.BACK);
          // });
        })
        .catch((error) => {
          ui.alertDialog(
            ...
            (dialog: HTMLElement) => {
              ui.remove(dialog);
              if (LiveStreamManager.PRIMARY_CAMERA_NAME !== 'P_CAM') {
                const message = { mode: 'back_camera_restart', text: 'back camera restart' };
    ...
    if (button) {
      ui.click(button, async () => {
        ...
        this.subscribe(SDK_EVENT.SECOND_STREAM, function (user_name: string, stream: MediaStream) {
          ...
          cameraSetup.hideCameraSelectPage();
          clearInterval(self.streamBackInterval!);
        });
        ...
        this.subscribe(SDK_EVENT.SECOND_STREAM_DISCONNET, function (user_name: string) {
          ...
          if(cameraName == 'B_CAM'){
            ...
            if (self.cameraRevokePopup == null) {
              if (LiveStreamManager.CAMERA.BACK.external == true) {
  ...
  async setRoomStream(stream: MediaStream, aiStart = 1) {
    ...
    cameraSetup.setAttemptData(configrationManager.roomAttemptNo, 2);
    if (stream) {
      ...
      if (aiStart == 1) {
        ...
        utility.log('Starting AI for Back camera');
  ...
  waitForPosition(video: HTMLVideoElement): Promise<any> {
    return new Promise((resolve) => {
      ai.secondaryCameraPosition(video, this.camType, (message: any) => {
        ...
        if(this.ntwDisconnectBack == true){
          return;
        }
        ...
        if (message.status_code === 155 || this.flag) {
          ...
          if (this.validPositionCount > 2 || this.flag) {
            ...
            ai.stopSecondaryCameraPosition((msg: any) =>
              utility.log(msg, 'stop_back_camera_position')
            );
  ...
  waitForValidation(video: HTMLVideoElement): Promise<any> {
    return new Promise((resolve) => {
      ai.secondaryCameraPositionValidate(video, (message: any) => {
        ...
        if (message.status_code === 171 || this.flag) {
  ...
  modeSelector(mode: string, message: Record<string, any>, user_name: string = '') {
    switch (mode) {
      ...
      case 'cam_setup_audioText':
        ...
        break;
      case 'sending_stream':
        this.streamBackInterval = setInterval(() => { 
      ...
      case 'getting_stream':
        ...
        peer.connect(user_name,LiveStreamManager.CAMERA.BACK);
        break;
      default:
  ...
  completeCameraSetup(log: boolean = true) {
    ...
    stepUIManager.insertText(
      ...
      ui.translations.status.back_cameraSetupFinish
    );
    if (configrationManager.valueMap.additional_cam.data.ai_enable.data.ai_front_view_cam) {
      stepUIManager.insertText(
    ...
    const camSetUp = ui.id('thinkX_backCameraSetup_Start');
    if (camSetUp) {
  ...
  closeTrigger() {
    ...
    AddCamBackView.stepManager.closeApplication();
    utility.log('close');
  }
  ...
  cameraRevoke(): void {
    ...
    ai.stopSecondaryCameraPosition((msg: any) => {});
    ai.stopSecondaryCameraPositionValidate((msg: any) => {});
    utility.log('back camera revoke alert show');
...
export const addCamBackView = new AddCamBackView();
